...
class ProjectSummary {
  ...
  int get progressValue {
    ...
    final completed = phases.where((p) => p.status?.toLowerCase() == 'completed').length;
    return ((completed / phases.length) * 100).round();
  }

  int get budgetUtilized {
    if (financials == null || financials!.totalContractValue == 0) return 0;
    return ((financials!.totalEarned / financials!.totalContractValue) * 100).round();
  }

  bool get isLoading => project == null;
}

final projectSummaryProvider = FutureProvider.family<ProjectSummary, String>((ref, projectId) async {
