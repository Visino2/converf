...
class _DailyReportFormScreenState extends ConsumerState<DailyReportFormScreen> {
  ...
  Future<void> _loadReportData() async {
    final detailProvider = dailyReportDetailProvider((
      projectId: widget.projectId,
      reportId: widget.reportId!,
    ));

    // Only show full-screen loader if we don't have any cached data yet
    if (!ref.read(detailProvider).hasValue) {
      setState(() => _isLoading = true);
    }

    try {
      final report = await ref.read(detailProvider.future);
      ...
      if (mounted) {
        setState(() {
          ...
          if (report.temperatureC != null) {
            _weatherData['temperature_c'] = report.temperatureC;
          }
          if (report.weatherCondition != null) {
            _weatherData['weather_condition'] = report.weatherCondition;
          }
          ...
          if (report.laborCount != null) {
            _resourcesData['labor_count'] = report.laborCount;
          }
          if (report.equipmentOperatingCount != null) {
            _resourcesData['equipment_operating_count'] =
                report.equipmentOperatingCount;
          }
          if (report.deliveriesCount != null) {
            _resourcesData['deliveries_count'] = report.deliveriesCount;
          }

          _activityUpdates = report.activityUpdates
              .map(
                (a) => {
          ...
          _tomorrowPlan = report.tomorrowPlan
              .map(
                (a) => {
                  'project_activity_id': a.projectActivityId,
  ...
  Widget build(BuildContext context) {
    if (_isLoading &&
      ...
      // Only show full screen loader during initial load of existing report
      return Scaffold(
    ...
    return Scaffold(
      ...
      body: metaAsync.when(
        ...
        loading: () {
          if (metaAsync.hasValue) {
            return _buildFormContent(metaAsync.value!);
          }
          return const Center(
            child: CircularProgressIndicator(color: Color(0xFF276572)),
          );
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  err.toString().contains('schedule')
                      ? 'No project schedule found. A schedule is required to log daily reports.'
                ...
                ElevatedButton(
                  onPressed: () => ref.invalidate(dailyReportFormMetaProvider),
        ...
        data: (meta) => _buildFormContent(meta),
      ),
    );
  }
  ...
  Widget _buildFormContent(DailyReportFormMeta meta) {
    ...
    if (_activityUpdates.isEmpty &&
        meta.todayCriticalActivities.isNotEmpty &&
        widget.reportId == null) {
      _activityUpdates = meta.todayCriticalActivities
          .map(
            (a) => {
    ...
    if (_tomorrowPlan.isEmpty &&
        meta.tomorrowCriticalActivities.isNotEmpty &&
        widget.reportId == null) {
      _tomorrowPlan = meta.tomorrowCriticalActivities
          .map(
            (a) => {
              'project_activity_id': a['project_activity_id'],
      ...
      children: [
        ...
        _buildDropdownField(
          ...
          _weatherData['weather_condition'] ??
              meta.defaults['weather_condition'],
          (meta.options['weather_conditions'] as List?)
                  ?.map((e) => e.toString())
                  .toList() ??
              ['clear', 'light_rain', 'heavy_rain', 'storm', 'extreme_heat'],
          (val) {
          ...
          children: [
            Expanded(
              child: _buildTextField(
                'Temperature (°C)',
                initialValue:
                    _weatherData['temperature_c']?.toString() ??
                    meta.defaults['temperature_c']?.toString(),
                onChanged: (val) {
            ...
            Expanded(child: _buildHoursLostField()),
          ],
        ),
        const SizedBox(height: 16),
        _buildDropdownField(
          'Concrete Pour Possible?',
          _concretePourPossible,
          (meta.options['yes_no_na'] as List?)
                  ?.map((e) => e.toString())
                  .toList() ??
              ['yes', 'no', 'na'],
          (val) {
        ...
        _buildDropdownField(
          'Labor Sufficiency',
          _laborSufficiency,
          (meta.options['labor_sufficiency'] as List?)
                  ?.map((e) => e.toString())
                  .toList() ??
              ['yes', 'no', 'overstaffed'],
          (val) {
        ...
        Row(
          children: [
            Expanded(
              child: _buildTextField(
                'Labor Count',
                initialValue: _resourcesData['labor_count']?.toString(),
                onChanged: (val) {
                  setState(
                    () => _resourcesData['labor_count'] = int.tryParse(val),
                  );
                },
              ),
            ),
            ...
            Expanded(
              child: _buildTextField(
                'Equipment Running',
                initialValue: _resourcesData['equipment_operating_count']
                    ?.toString(),
                onChanged: (val) {
                  setState(
                    () => _resourcesData['equipment_operating_count'] =
        ...
        _buildTextField(
          'Deliveries Count',
          initialValue: _resourcesData['deliveries_count']?.toString(),
          onChanged: (val) {
            setState(
              () => _resourcesData['deliveries_count'] = int.tryParse(val),
            );
          },
        ),
        SwitchListTile(
  ...
  Widget _buildActivityItem(
    ...
    final title =
        metaActivity?['title'] ?? update['title'] ?? 'Activity Update';
            ...
            TextButton.icon(
              onPressed: () {
                setState(() {
                  _issues.add({
                    'issue_type':
                        (meta.options['issue_types'] as List?)?.first ??
                        'other',
                    'impact_days': 0,
                    'resolution_type': '',
                    'resolution_note': '',
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: _buildDropdownField(
                        'Issue Type',
                        issue['issue_type'],
                        (meta.options['issue_types'] as List?)
                                ?.map((e) => e.toString())
                                .toList() ??
                            [
                              'weather_delay',
                ...
                Row(
                  children: [
                    Expanded(
                      child: _buildTextField(
                        'Impact (Days)',
                        initialValue: issue['impact_days'].toString(),
                        onChanged: (val) {
                          setState(
                            () => issue['impact_days'] = int.tryParse(val) ?? 0,
                          );
                        },
                      ),
                    ),
                ...
                _buildDropdownField(
                  ...
                  [
                    '',
                    ...(meta.options['resolution_types'] as List?)
                            ?.map((e) => e.toString())
                            .toList() ??
                        [
                          'waiting_on_client',
                      ...
                      (val) {
                        if (val == null || val.isEmpty) {
                          ...
                          final attendee = meta.issueAssignees.firstWhere(
                            (a) =>
                                '${a['first_name']} ${a['last_name']}' == val,
                            orElse: () => null,
                          );
                          setState(
                            () => issue['assigned_to'] = attendee?['id'],
                          );
                        }
                      },
                    );
                ...
                _buildTextField(
                  'Notes/Resolution',
                  initialValue: issue['resolution_note'],
                  onChanged: (val) {
  ...
  Widget _buildTomorrowItem(
    ...
    final title =
        metaActivity?['title'] ?? update['title'] ?? 'Planned Activity';
          ...
          Row(
            children: [
              Expanded(
                child: _buildTextField(
                  'Start Time',
                  initialValue: update['start_time'] ?? '07:00',
                  onChanged: (val) {
              ...
              Expanded(
                child: _buildTextField(
                  'Crew Size',
                  initialValue: update['crew_size']?.toString() ?? '5',
                  onChanged: (val) {
                    setState(
                      () => update['crew_size'] = int.tryParse(val) ?? 5,
                    );
                  },
                ),
              ),
  ...
  Widget _buildDropdownField(
    ...
    String? effectiveValue = options.any((o) => o == value)
        ? value
        : (options.contains('')
              ? ''
  ...
  Future<void> _saveDraft() async {
    ...
    try {
      ...
      await ref
          ...
          .upsertDraft(widget.projectId, payload);
  ...
  Future<void> _submitReport() async {
    ...
    try {
      ...
      if (reportId != null) {
        await ref
            .read(dailyReportActionProvider.notifier)
            .submitReport(widget.projectId, reportId);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
