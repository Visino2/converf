...
class _DailyReportsScreenState extends ConsumerState<DailyReportsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: SvgPicture.asset('assets/images/filter.svg', colorFilter: const ColorFilter.mode(Colors.black, BlendMode.srcIn)),
  ...
  List<dynamic> _resolveMissingReports(Project? project, List<DailyReport> reports) {
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          SvgPicture.asset('assets/images/document-1.svg', width: 64, height: 64, colorFilter: const ColorFilter.mode(Color(0xFFD0D5DD), BlendMode.srcIn)),
          ...
          ElevatedButton(
            ...
            child: const Text('Create Report', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
          ),
        ],
      ),
    );
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                  ...
                  style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFFB42318)),
                ),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  decoration: BoxDecoration(color: const Color(0xFFFEE4E2), borderRadius: BorderRadius.circular(12)),
                  child: const Text(
                    'NO REPORT',
                    style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Color(0xFFB42318)),
                  ),
                ),
              ],
            ),
            ...
            SizedBox(
              ...
              child: OutlinedButton(
                onPressed: () => _createNewReportForDate(missingData['reportDate']),
                style: OutlinedButton.styleFrom(
                  side: const BorderSide(color: Color(0xFFFECDCA)),
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
                ),
                child: const Text('Create Report', style: TextStyle(color: Color(0xFFB42318), fontSize: 13, fontWeight: FontWeight.bold)),
              ),
            ),
          ],
        ),
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    if (report.weatherCondition != null) ...[
                      ...
                      const SizedBox(width: 8),
                      Icon(_getWeatherIcon(report.weatherCondition), size: 14, color: const Color(0xFF475467)),
                ...
                Row(
                  children: [
                    _buildStatItem(Icons.engineering, '${report.activityUpdates.length} Activities'),
                    const SizedBox(width: 16),
                    _buildStatItem(Icons.report_problem_outlined, '${report.issues.length} Issues'),
                    const Spacer(),
  ...
  IconData _getWeatherIcon(String? condition) {
    switch (condition?.toLowerCase()) {
      ...
      case 'light_rain': return Icons.cloud_outlined;
      case 'heavy_rain': return Icons.umbrella_outlined;
      case 'storm': return Icons.thunderstorm_outlined;
      case 'extreme_heat': return Icons.wb_sunny;
  ...
  Future<void> _submitReport(DailyReport report) async {
    try {
      await ref.read(dailyReportActionProvider.notifier).submitReport(widget.projectId, report.id);
      if (mounted) {
  ...
  void _createNewReportForDate(String date) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DailyReportFormScreen(projectId: widget.projectId, initialDate: date),
      ),
    );
  }
  ...
  void _createNewReport() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DailyReportFormScreen(projectId: widget.projectId),
      ),
    );
  }
